...
func BindRoutesBase(server *fiber.App, imageSource imagesources.ImageSource) error {
	...
	server.Post("/images", func(c *fiber.Ctx) error {
	...
	server.Put("/images", func(c *fiber.Ctx) error {
		// Get the uploaded file
		file, err := c.FormFile("image")
		if err != nil {
			log.Errorw("failed to get uploaded file", "error", err.Error())
